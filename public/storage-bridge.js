...
(function() {
  ...
  async function initializeUser() {
    ...
    let savedUserId = sessionStorage.getItem('gamehole_user_id');

    if (!savedUserId) {
      // Show login UI (if available) or use prompt
      if (window.GameholeLoginUI && window.GameholeLoginUI.show) {
        savedUserId = await window.GameholeLoginUI.show();
      } else {
      ...
      if (savedUserId && savedUserId.trim()) {
        ...
        sessionStorage.setItem('gamehole_user_id', savedUserId);
        userId = savedUserId;
  ...
  window.GameholeStorageBridge = {
    changeUser: function(newUserId) {
      if (newUserId && newUserId.trim()) {
        userId = newUserId.trim().toLowerCase();
        sessionStorage.setItem('gamehole_user_id', userId);
    ...
    logout: function() {
      sessionStorage.removeItem('gamehole_user_id');
      userId = null;
      isInitialized = false;
