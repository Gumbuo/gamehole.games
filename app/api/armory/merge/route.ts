...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check Gold balance
    const currentGold = saveState.gold || 0;
    if (currentGold < MERGE_COST) {
      return NextResponse.json({
        success: false,
        error: `Insufficient Gold. Need ${MERGE_COST}G, have ${currentGold}G`,
      });
    }
    ...
    // Deduct Gold
    saveState.gold = currentGold - MERGE_COST;
