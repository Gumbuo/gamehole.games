...
export default function AlienArmory() {
  ...
  // Shop: Buy materials with Gold
  const buyMaterial = async (resourceId: RawResourceKey, quantity: number) => {
    if (!address || !saveState) return;
    const cost = MATERIAL_COSTS[resourceId] * quantity;
    if ((saveState.gold || 0) < cost) {
      showNotification(`Insufficient Gold! Need ${cost}G`, "error");
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setSaveState((prev) => prev ? { ...prev, resources: data.data.resources, gold: data.data.newGoldBalance } : null);
  ...
  const speedUpCraft = async (jobId: string, type: "half" | "instant") => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setSaveState((prev) => prev ? { ...prev, gold: data.data.newGoldBalance } : null);
        // Reload to get updated queues
        loadSaveState();
        showNotification(`Sped up! -${data.data.goldSpent}G`, "success");
      } else {
  ...
  const sellItem = async (itemId: string, quantity: number, rarity: RarityTier = 'common') => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setSaveState((prev) => prev ? { ...prev, inventory: data.data.inventory, gold: data.data.newGoldBalance } : null);
        showNotification(`Sold! +${data.data.goldEarned}G`, "success");
      } else {
  ...
  const upgradeStation = async (stationId: StationId) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setSaveState((prev) => prev ? { ...prev, stationLevels: data.data.stationLevels, gold: data.data.newGoldBalance } : null);
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ color: THEME.colors.primary, fontSize: "20px", fontWeight: "bold" }}>
              {(saveState?.gold || 0).toLocaleString()}G
            </div>
          </div>
        </div>
      </div>
                ...
                <>
                        ...
                        >
                          ...
                          {!isComplete && (
                            <div style={{ display: "flex", gap: "4px", marginTop: "4px" }}>
                              ...
                              >
                                ½ Time ({calculateSpeedUpCost(remaining, "half")}G)
                              </button>
                              <button
                              ...
                              >
                                Instant ({calculateSpeedUpCost(remaining, "instant")}G)
                              </button>
                            </div>
                          )}
                        </div>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    ...
                    {level < station.maxLevel && (
                      ...
                      >
                        ⬆️ {station.upgradeCosts[level]}G
                      </button>
                    )}
                  </div>
                </>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: "16px", color: THEME.colors.secondary }}>
              Your Gold: {(saveState?.gold || 0).toLocaleString()}
            </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: THEME.colors.secondary, fontSize: "12px" }}>
                      {MATERIAL_COSTS[resourceId]}G each
                    </div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: "8px" }}>
                  {[1, 5, 10].map((qty) => (
                    <button
                      ...
                      disabled={(saveState?.gold || 0) < MATERIAL_COSTS[resourceId] * qty}
                      style={{
                        background: (saveState?.gold || 0) >= MATERIAL_COSTS[resourceId] * qty
                        ...
                        color: "#000",
                        fontWeight: "bold",
                        cursor: (saveState?.gold || 0) >= MATERIAL_COSTS[resourceId] * qty ? "pointer" : "not-allowed",
                        fontFamily: "Orbitron, sans-serif",
                        fontSize: "11px",
                      }}
                    >
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        <div style={{ display: "flex", gap: "4px" }}>
                          ...
                          >
                            SELL (+{rarityItem.sellValue}G)
                          </button>
                        </div>
                        {canMerge && (
                          <button
                            onClick={() => mergeItem(slot.itemId, rarity)}
                            disabled={(saveState?.gold || 0) < MERGE_COST}
                            title={nextRarity ? `Merge 2x into 1x ${RARITY_NAMES[nextRarity]} (${MERGE_COST}G)` : ""}
                            style={{
                              background: (saveState?.gold || 0) >= MERGE_COST
                              ...
                              color: "#000",
                              fontWeight: "bold",
                              cursor: (saveState?.gold || 0) >= MERGE_COST ? "pointer" : "not-allowed",
                              fontFamily: "Orbitron, sans-serif",
                              fontSize: "10px",
                            }}
                          >
